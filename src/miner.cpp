...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AdirondackMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AdirondackMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AdirondackMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("adirondack-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running AdirondackMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
